...
\begin{align*}
    & \text{presses}:& A \rightarrow 2^P,& \text{ presses}(a) =& P_a \\
    & \text{layers}:& A \rightarrow 2^L,& \text{ layers}(a) =& L_a \\
    & \text{components}:& A \rightarrow 2^C,& \text{ components}(a) =& C_a \\
\end{align*}
...
Bei Ziel 1, der Minimierung der Anzahl unverplanter Komponenten ist das Optimum immer Null, wenn der Auftrag erfüllbar ist.
Also:
\begin{align*}
...
% TODO: Naja, so trivial ist der Beweis nicht! Der sollte hier besser noch hingeschriebenw werden!
